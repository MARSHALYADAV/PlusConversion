...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const pageSorter = new PageSorter('page-sorter-container', {
        ...
        onProcess: async (pagesToKeep) => {
            pageSorter.hide();
            progressUI.show('Processing PDF with removed pages...');
            try {
                await executeRemove(currentArrayBuffer, pagesToKeep, originalFilename, progressUI, resultCard);
            } catch (err) {
                console.error(err);
                progressUI.hide();
                pageSorter.show();
                Toast.show('Failed to remove pages: ' + err.message, 'error');
            }
        }
    });
...
async function executeRemove(arrayBuffer, pagesToKeep, filename, progressUI, resultCard) {
    progressUI.update('Loading PDF binary...', 20);
    const { PDFDocument } = PDFLib;

    const originalDoc = await PDFDocument.load(arrayBuffer, { ignoreEncryption: true });
    
    progressUI.update('Creating cleaned document...', 50);
    ...
    const outFilename = filename.replace(/\.pdf$/i, '_removed.pdf');
    ...
    Toast.show('Pages removed successfully!', 'success');
}
