...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('submit-btn')?.addEventListener('click', async () => {
        if (!selectedFile) { Toast.show('Please select a PowerPoint file first.', 'warning'); return; }

        ui.showProcessing('Converting Presentation slides to PDF…');

        try {
            const data = await convertOfficeToPdf(selectedFile, 'ppt');
            rc.show(data);
            ui.showResult();
            Toast.show('Presentation converted successfully!', 'success');
        } catch (err) {
            ui.reset();
